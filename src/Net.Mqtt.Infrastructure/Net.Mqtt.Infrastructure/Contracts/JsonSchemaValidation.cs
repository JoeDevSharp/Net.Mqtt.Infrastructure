...
/// <summary>Represents validation error.</summary>
public sealed record ValidationError(string Path, string Message);
/// <summary>Represents validation result.</summary>
...
public sealed class InMemoryJsonSchemaResolver : IJsonSchemaResolver
{
    ...
    /// <summary>Adds the add operation.</summary>
...
/// <summary>Represents file json schema resolver.</summary>
...
/// <summary>Represents http json schema resolver.</summary>
...
/// <summary>Represents composite json schema resolver.</summary>
...
/// <summary>Represents caching json schema resolver.</summary>
...
/// <summary>Represents json schema event data validator.</summary>
public sealed class JsonSchemaEventDataValidator(IJsonSchemaResolver resolver) : IEventDataValidator
{
    ...
    private static void ValidateNode(JsonElement rootSchema, JsonElement schema, JsonElement value, string path, List<ValidationError> errors)
    {
        ...
        {
            errors.Add(new(path, $"Expected JSON type '{FormatExpectedType(type)}', found '{value.ValueKind}'."));
            return;
        }
    ...
    private static bool MatchesType(JsonElement value, JsonElement type)
    {
        if (type.ValueKind == JsonValueKind.String)
            return MatchesSingleType(value, type.GetString());

        if (type.ValueKind == JsonValueKind.Array)
            return type.EnumerateArray()
                .Any(item => item.ValueKind == JsonValueKind.String && MatchesSingleType(value, item.GetString()));

        return true;
    }

    private static bool MatchesSingleType(JsonElement value, string? type) => type switch
    {
    ...
    private static string FormatExpectedType(JsonElement type) =>
        type.ValueKind == JsonValueKind.Array
            ? string.Join(" | ", type.EnumerateArray().Select(item => item.GetString()))
            : type.GetString() ?? type.ValueKind.ToString();
}

/// <summary>Represents json schema resolution exception.</summary>
public sealed class JsonSchemaResolutionException : IOException
{
    /// <summary>Executes the json schema resolution exception operation.</summary>
...
/// <summary>Represents event data validation exception.</summary>
